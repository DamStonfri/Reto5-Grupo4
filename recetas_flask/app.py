...
def recetas():
    ...
    if filtro == "vegana":
        # Solo recetas con ingredientes vegetales y sin proteinas ni carbohidratos
        cursor.execute("""
        ...
        # Recetas con proteina y vegetal a la vez
        cursor.execute("""
            SELECT r.id, r.titulo, r.descripcion, r.usuario_id, r.imagen, u.nombre AS autor, r.creada_en
            FROM recetas r JOIN usuarios u ON r.usuario_id = u.id
...
def detalle_receta(id):
    ...
    # Totales mediante funciones almacenadas
    cursor2 = db.cursor()
...
def editar_receta(receta_id):
    ...
    if request.method == "POST":
        ...
        # Borra los anteriores y reinserta los nuevos
...
def menus():
    ...
    # Carga las recetas de cada menu
...
def eliminar_menu(menu_id):
    ...
    db = conectar()
    cursor = db.cursor()
    # Borra primero las relaciones y luego el menu
